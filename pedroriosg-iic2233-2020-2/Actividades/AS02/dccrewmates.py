...
class DCCrewmates(Thread):
    ...
    def run(self):
        ...
        for thread_tripulante in self.tripulantes:
            thread_tripulante.start()
        self.impostor.start()
        for thread_tripulante in self.tripulantes:
            thread_tripulante.join()
        self.evento_termino.set()
        self.impostor.join()
        gana_impostor = False
        for key in self.diccionario_tareas:
            estado = self.diccionario_tareas[key]["realizado"]
            if not estado:
                print("Ha ganado el Impostor")
                gana_impostor = True
                break
        if not gana_impostor:
            print("Han ganado los tripulantes")
        print("El color del Impostor es", self.impostor.color)
