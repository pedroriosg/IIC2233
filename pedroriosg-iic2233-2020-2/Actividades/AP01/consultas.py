...
def obtener_comida(id_, comidas):
    ...
    instancia_comida = filter(lambda comida: comida.id == id_, comidas)
    return list(instancia_comida)[0]


def obtener_comidas(ids, comidas):
    # Aquí debes utilizar map y la función id_a_comida(id, comidas)
    mapeo = map(lambda id_: obtener_comida(id_, comidas), ids)
    return list(mapeo)
...
def precio_total(comidas_cliente):
    ...
    total = reduce(lambda x, y: x.precio + y.precio, comidas_cliente)
    return total
