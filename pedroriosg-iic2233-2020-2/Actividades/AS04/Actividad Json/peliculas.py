...
def cargar_peliculas(ruta):
    ...
    ### COMPLETAR ###
    with open(ruta, "rt") as peliculas_file:
        return json.load(peliculas_file, object_hook=desencriptado)
    

def desencriptado(diccionario):

    ### COMPLETAR ###
    dict_desencriptado = dict()
    for nombre_pelicula in diccionario:
        name = desencriptar(nombre_pelicula)
        lista = []
        numero = diccionario[nombre_pelicula][0]
        lista.append(numero)
        datos = diccionario[nombre_pelicula][1:]
        for dato in datos:
            dato_desencriptado = desencriptar(dato)
            lista.append(dato_desencriptado)
        dict_desencriptado[name] = lista
