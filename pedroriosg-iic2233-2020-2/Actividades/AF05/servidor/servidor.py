...
class Servidor:
    ...
    def aceptar_clientes(self):
        ...
        # Completar
        while True:
            socket_cliente, direccion = self.socket_server.accept()
            for player in self.lista_jugadores:
                if player.socket_cliente is None:
                    player.socket_cliente = socket_cliente
                    thread_jugador_escuchar = threading.Thread(target=self.escuchar_cliente, args=(player, ), daemon=True)
                    thread_jugador_escuchar.start()
                    break
    ...
    def escuchar_cliente(self, jugador):
        ...
        try:
            ...
            while True:
                ...
                respuestas = self.logica.manejar_mensaje(mensaje, jugador, self.lista_jugadores)
                self.enviar_lista_respuestas(jugador, respuestas)
    ...
    def enviar(self, mensaje, socket_cliente):
        ...
        largo_msg = len(msg).to_bytes(5, byteorder='little')
        socket_cliente.sendall(largo_msg + msg)
    ...
    def recibir(self, socket_cliente):
        ...
        while len(bytes_leidos) < largo_mensaje:
            bytes_leer = min(64, largo_mensaje - len(bytes_leidos))
