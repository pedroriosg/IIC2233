...
class BolsilloCriaturas(list):
    ...
    def append(self, criatura):
        ...
        if len(self) < 6:
            ...
            print("Error, se llegó al límite. No agregar")

    def cantidad_criaturas_estrella(self):
        # Completar
        cantidad = 0
        for criatura in self:
            total = int(criatura.hp_base) + int(criatura.atk) + int(criatura.sp_atk) + int(criatura.defense)
            if total > 400:
                cantidad += 1
        return cantidad
    ...
    def __add__(self, bolsillo_enemigo):
        ...
        posicion = 0
        total = 0
        for indice in range(len(bolsillo_enemigo)):
            suma = bolsillo_enemigo[indice].atk + bolsillo_enemigo[indice].sp_atk
            if suma > total:
                posicion = indice
                total = suma
